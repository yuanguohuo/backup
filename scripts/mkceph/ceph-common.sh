...
check_host() {
    ...
    if [ -z "$user" ]; then
        ssh="ssh $SSH_PORT_OPT $host"
    else
        ssh="ssh $SSH_PORT_OPT $user@$host"
    fi

    rootssh="ssh $SSH_PORT_OPT root@$host"
...
do_cmd_truck() {
    if [ -z "$ssh" ] ; then
        ...
        echo "In do_cmd_truck(): Run command \"$1\" on host $host remotely"
        python /opt/truck/remote.py call -H $host -c "ulimit -c unlimited ; $1" -t 600
    fi
}
...
do_root_cmd_truck() {
    if [ -z "$ssh" ]; then
        ...
        echo "In do_root_cmd_truck(): Run command \"$1\" on host $host remotely"
        python /opt/truck/remote.py call -H $host -c "ulimit -c unlimited ; $1" -t 600
    fi
}
