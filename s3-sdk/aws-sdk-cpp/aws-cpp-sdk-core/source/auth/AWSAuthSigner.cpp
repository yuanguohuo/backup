...
static Aws::String AWS2_CanonicalizedResource(HttpRequest& request)
{
    Aws::StringStream signingStringStream;
    ...
    Aws::String encodedPath =  uriCpy.GetURLEncodedPath();

    if (encodedPath.size() > 0)
    {
      signingStringStream << encodedPath;
    }
    else
    {
      signingStringStream << "/";
    ...
    bool firstQParam = true;

    QueryStringParameterCollection queryParams = request.GetQueryStringParameters();
    for(int i=0; i<16; i++)
    {
      QueryStringParameterCollection::const_iterator itr = queryParams.find(subresources[i]);
      if (itr != queryParams.end())
      {
        if (firstQParam) 
        {
          firstQParam = false;
        ...
        signingStringStream << itr->first;

        //Yuanguo: See aws-cpp-sdk-core/source/http/URI.cpp:GetQueryStringParameters()
        //     If the queryParam has not "=", such as "uploads" in "/testbuck1/ObjCCC?uploads"
        //        size_t locationOfEquals = keyValuePair.find('=');   
        //     locationOfEquals will get '-1', as a result,  "value" will be the
        //     same as "key",  and "key=key" will be inserted into parameterCollection
        // So, if "itr->second() == itr->first()" here, we should only add "first".
        ...
        {
          signingStringStream << "=" << itr->second;
        }
      }
    }
...
static Aws::String AWS2_CanonicalizeHeaders(HttpRequest& request)
{
    Aws::StringStream signingStringStream;

    Http::HeaderValueCollection headers = CanonicalizeHeaders(request.GetHeaders());

    Aws::Map<Aws::String, Aws::String> xamzHeaders;

    for(Http::HeaderValueCollection::const_iterator itr=headers.begin(); itr!=headers.end(); ++itr)
    {
      Aws::String name = Aws::Utils::StringUtils::ToLower(itr->first.c_str());
      const Aws::String& value = itr->second;
    ...
    //Yuanguo: xamzHeaders is std:map, which is ordered; thus we
    //don't need to sort it;

    for(Aws::Map<Aws::String, Aws::String>::const_iterator itr=xamzHeaders.begin(); itr!=xamzHeaders.end(); ++itr)
    {
      signingStringStream << itr->first << ":" << itr->second << NEWLINE;
    }
...
bool AWSAuthV2Signer::SignRequest(Aws::Http::HttpRequest& request) const
{
    ...
    if (contentMD5.size() > 0)
    {
      signingStringStream << contentMD5;
    }
    signingStringStream << NEWLINE;

    if (contentType.size() > 0)
    {
      signingStringStream << contentType;
    }
    signingStringStream << NEWLINE;

    DateTime now = GetSigningTimestamp();
    Aws::String dateHeaderValue = now.ToGmtString("%a, %d %b %Y %T %z");

    request.SetHeaderValue(AWS_DATE_HEADER, dateHeaderValue);
    signingStringStream << NEWLINE;

    Aws::String canonicalizeHeaders = AWS2_CanonicalizeHeaders(request);
    Aws::String canonicalizedResource = AWS2_CanonicalizedResource(request);

    signingStringStream << canonicalizeHeaders << canonicalizedResource; 

    Aws::String stringToSign = signingStringStream.str();
    Aws::String secKey =  credentials.GetAWSSecretKey();

    ByteBuffer finalSigningDigest = hmac_sh1(
                      ByteBuffer((unsigned char*)stringToSign.c_str(), stringToSign.length()), 
                      ByteBuffer((unsigned char*)secKey.c_str(), secKey.length()));

    Aws::String finalSignature = HashingUtils::Base64Encode(finalSigningDigest);
...
bool AWSAuthV2Signer::PresignRequest(Aws::Http::HttpRequest& request, long long expirationInSeconds) const
{
  AWS_UNREFERENCED_PARAM(request); 
  AWS_UNREFERENCED_PARAM(expirationInSeconds); 

  return true;
}
bool AWSAuthV2Signer::PresignRequest(Aws::Http::HttpRequest& request, const char* region, long long expirationInSeconds) const
{
  AWS_UNREFERENCED_PARAM(request); 
...
bool AWSAuthV2Signer::PresignRequest(Aws::Http::HttpRequest& request, const char* region, const char* serviceName, long long expirationInSeconds) const
{
  AWS_UNREFERENCED_PARAM(request); 
  AWS_UNREFERENCED_PARAM(region); 
...
ByteBuffer AWSAuthV2Signer::hmac_sh1(const ByteBuffer& stringToSign,  const ByteBuffer & secretKey) const
{
  AWS_UNREFERENCED_PARAM(stringToSign); 
  AWS_UNREFERENCED_PARAM(secretKey); 

    auto hashResult = m_HMAC->Calculate(stringToSign, secretKey);
    if (!hashResult.IsSuccess())
    {
        AWS_LOGSTREAM_ERROR(v2LogTag, "Unable to hmac (sha1) final string");
        return ByteBuffer((unsigned char*)"", 0);
    }
