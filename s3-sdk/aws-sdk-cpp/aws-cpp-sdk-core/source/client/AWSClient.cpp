...
Aws::Client::AWSAuthSigner* AWSClient::GetAuthSignerByName(const Aws::String& AuthSignerName) const
{
    ...
    if (iter == m_signerMap.end()) 
    {
        ...
        //         Several places hardcoded AuthSignerName to be DEFAULT_AUTHV4_SIGNER;
        //         I don't want to change all thoses places, so I just make a trick here
        //         to let it return DEFAULT_AUTHV2_SIGNER if there is;
        if (m_signerMap.begin() != m_signerMap.end()) 
        {
          //if the map is not empty, return the first one; when we use AWS Signatrue V2, 
          //DEFAULT_AUTHV2_SIGNER is the only signer in the map, so it's returned;
          return m_signerMap.begin()->second.get();
        }
