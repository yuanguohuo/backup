...
function _M.set_keepalive(self, ...)
    ...
    if self.keepalive == true then
        local res, err= sock:setkeepalive(...)
        if not res then
            ngx_log(ngx.WARN, "RequestID=", ngx.ctx.reqid, " ", "sock setkeepalive err :" .. (err or "nil"))
            sock:close()
        end
    else
...
local function _receive_status(sock)
    repeat
        ...
        if not line then
            ngx_log(ngx_ERR, "RequestID=", ngx.ctx.reqid, " sock:receive failed ",  err)
        ...
        if not content_length and max_chunk_size then
            ...
            repeat
                ...
                if not max_chunk_size then
                    ngx_log(ngx_ERR, "RequestID=", ngx.ctx.reqid, " ",  "Buffer size not specified, bailing")
                    break
                end
            until not str
...
function _M.send_request(self, params)
    ...
    ngx_log(ngx_DEBUG, "RequestID=", ngx.ctx.reqid, " ",  "\n", req)
...
function _M.read_response(self, params)
    ...
    if not status then
        ...
        if not status then
            ngx_log(ngx_ERR, "RequestID=", ngx.ctx.reqid, " _receive_status failed ",  err)
            return nil, err
        end
    end
    ...
    if not res_headers then
        ngx_log(ngx_ERR, "RequestID=", ngx.ctx.reqid, " _receive_headers failed ",  err)
        return nil, err
    end
    ...
    if err then
        ngx_log(ngx_ERR, "RequestID=", ngx.ctx.reqid, " _body_reader failed ",  err)
        return nil, err
    else
        return {
            status = status,
        ...
        responses[i] = setmetatable({
            ...
            __index = function(t, k)
                ...
                if t.response_read == false then
                    ...
                    if not res then
                        ngx_log(ngx_ERR, "RequestID=", ngx.ctx.reqid, " ",  err)
                    else
...
function _M.proxy_response(self, response, chunksize)
    if not response then
        ngx_log(ngx_ERR, "RequestID=", ngx.ctx.reqid, " ",  "no response provided")
        return false
    end
