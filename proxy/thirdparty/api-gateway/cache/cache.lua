...
function _M:hasStore(store)
    if (store == nil) then
        ngx.log(ngx.DEBUG, "RequestID=", ngx.ctx.reqid, " ",  "Store to be compared is nil")
        return false
    end
    for _, cache_store in ipairs(self.__cache_stores) do
        if (cache_store == nil) then
            ngx.log(ngx.DEBUG, "RequestID=", ngx.ctx.reqid, " ",  "Store from cache store is nil" )
            return false
        end
        ngx.log(ngx.DEBUG, "RequestID=", ngx.ctx.reqid, " ",  "Comparing store named: ", store:getName(), " with existing store named: ", cache_store:getName())
...
function _M:addStore(store)
    if (store == nil) then
        ngx.log(ngx.WARN, "RequestID=", ngx.ctx.reqid, " ",  "Attempt to add a nil cache store")
        return nil
    end

    if (self:hasStore(store)) then
       ngx.log(ngx.WARN, "RequestID=", ngx.ctx.reqid, " ",  "Attempt to add a cache store that already exists: ", store:getName())
       return nil
    end
...
function _M:get(key)
    if (key == nil) then
        ngx.log(ngx.WARN, "RequestID=", ngx.ctx.reqid, " ",  "Attempting to get a nil key from cache")
        return nil, nil
    end
    local store_idx = 0
    for idx, cache_store in ipairs(self.__cache_stores) do
        ...
        if (val ~= nil) then
            ...
            ngx.log(ngx.DEBUG, "RequestID=", ngx.ctx.reqid, " ",  "Returning the cached value for ", tostring(key), " from ", tostring(cache_store:getName()))
...
function _M:put(key, value)
    if (key == nil) then
        ngx.log(ngx.WARN, "RequestID=", ngx.ctx.reqid, " ",  "Attempting to put a nil key in cache")
        return nil
    end
