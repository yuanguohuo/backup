...
function _M:getTTL(key,value)
    if (type(self.ttl) == "function") then
        ...
        if (result) then
            ngx.log(ngx.DEBUG, "RequestID=", ngx.ctx.reqid, " ",  "Key:[" , tostring(key), "] should expire in ", tostring(ttl_v) , "s from ", tostring(self:getName()))
            return tonumber(ttl_v)
        end
        ngx.log(ngx.WARN, "RequestID=", ngx.ctx.reqid, " ",  "Error calling ttl function:", tostring(ttl_v))
        return nil
    end
    ngx.log(ngx.DEBUG, "RequestID=", ngx.ctx.reqid, " ",  "Key:[" , tostring(key), "] should expire in ", tostring(self.ttl) , "s from ", tostring(self:getName()))
