...
local function update_healthy_upstream_in_cache(dict_name, upstream_name, healthy_upstream)
    local dict = ngx.shared[dict_name];
    if (nil ~= dict) then
        ngx.log(ngx.DEBUG, "RequestID=", ngx.ctx.reqid, " ",  "Saving a healthy upstream:", healthy_upstream, " in cache:", dict_name, " for upstream:", upstream_name)
    ...
    ngx.log(ngx.WARN, "RequestID=", ngx.ctx.reqid, " ",  "Dictionary ", dict_name,  " doesn't seem to be set. Did you define one ? ")
end
...
function _M:getHealthyServer(upstream_name)
    ...
    ngx.log(ngx.DEBUG, "RequestID=", ngx.ctx.reqid, " ",  "Looking up for a healthy peer in upstream:", upstream_name)
    ...
    for key,value in ipairs(upstream_health_result) do
        ...
        if(value == " DOWN\n" and upstream_health_result[key-1] ~= nil ) then
            ngx.log(ngx.WARN, "RequestID=", ngx.ctx.reqid, " ",  "Peer ", tostring(upstream_health_result[key-1]), " is down! Checking for backup peers.")
        end
    end

    ngx.log(ngx.ERR, "RequestID=", ngx.ctx.reqid, " ",  "All peers are down!")
