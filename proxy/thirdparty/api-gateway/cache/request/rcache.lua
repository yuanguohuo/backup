...
local function put(cache, key)
    ...
    if (not pcall_ok) then
        ngx.log(ngx.WARN, "RequestID=", ngx.ctx.reqid, " ",  "Could not read request body. Check the logs for more info.")
        return
    end
    --ngx.log(ngx.DEBUG, "RequestID=", ngx.ctx.reqid, " ",  "Storing value=", tostring(value), " into key=", tostring(key), " in cache")
    ngx.log(ngx.DEBUG, "RequestID=", ngx.ctx.reqid, " ",  "Storing key=", tostring(key), " in cache.")
    if (value ~= nil) then
        cache:put(tostring(key),value)
--        ngx.log(ngx.DEBUG, "RequestID=", ngx.ctx.reqid, " ",  "Stored value=", tostring(value), " with key=", tostring(key), " in cache")
        ngx.log(ngx.DEBUG, "RequestID=", ngx.ctx.reqid, " ",  "Stored key=", tostring(key), " in cache.")
        return ngx.HTTP_OK
    end
end
...
local function get(cache, key)
    ...
    if (val ~= nil) then
--        ngx.log(ngx.DEBUG, "RequestID=", ngx.ctx.reqid, " ",  "Found value=", tostring(val), " with key=", tostring(key), " stored in cache")
        ngx.log(ngx.DEBUG, "RequestID=", ngx.ctx.reqid, " ",  "Found key=", tostring(key), " stored in cache.")
        return ngx.HTTP_OK, val
    end
    ngx.log(ngx.DEBUG, "RequestID=", ngx.ctx.reqid, " ",  "No value in cache found for key=", tostring(key))
...
function _M:handleRequest(sr_method, cache, key)
    ngx.log(ngx.DEBUG, "RequestID=", ngx.ctx.reqid, " ",  " Handling ", sr_method , " for key [", tostring(key), "]")
