...
local function getRedisUpstream(upstream_name)
    ...
    ngx.log(ngx.DEBUG, "RequestID=", ngx.ctx.reqid, " ",  "Obtained Redis Host:" .. tostring(host) .. ":" .. tostring(port), " from upstream:", n)
    if (nil ~= host and nil ~= port) then
        return host, port
    end

    ngx.log(ngx.ERR, "RequestID=", ngx.ctx.reqid, " ",  "Could not find a Redis upstream.")
    return nil, nil
end
...
function _M:get(key)
    ...
    if ok then
        ...
        if (err) then
            ngx.log(ngx.WARN, "RequestID=", ngx.ctx.reqid, " ",  "Could not return a value for key=[", tostring(key), "].", err)
            return nil
        end
        if (redis_response == ngx.null) then
            ngx.log(ngx.DEBUG, "RequestID=", ngx.ctx.reqid, " ",  "key=[", tostring(key), "] not found in ", tostring(self:getName()))
            return nil
        end
        ngx.log(ngx.WARN, "RequestID=", ngx.ctx.reqid, " ",  "key=[", tostring(key), "] returned a value of type=", type(redis_response), " from ", tostring(self:getName()))
        return redis_response
    end
    ngx.log(ngx.WARN, "RequestID=", ngx.ctx.reqid, " ",  "Failed to read key " .. tostring(key) .. " from Redis cache:[", redis_host, ":", redis_port, "]. Error:", err)
    return nil
end
...
--    ngx.log(ngx.DEBUG, "RequestID=", ngx.ctx.reqid, " ",  "Storing in Redis the key [", tostring(key), "], expires in=", tostring(keyexpires), " s, value=", tostring(value))
    ngx.log(ngx.DEBUG, "RequestID=", ngx.ctx.reqid, " ",  "Storing in Redis the key [", tostring(key), "], expires in=", tostring(keyexpires), " s" )
    ...
    if ok then
        --ngx.log(ngx.DEBUG, "RequestID=", ngx.ctx.reqid, " ",  "WRITING IN REDIS JSON OBJ key=" .. key .. "=" .. value .. ",expiring in:" .. (keyexpires - (os.time() * 1000)) )
        ...
        --ngx.log(ngx.WARN, "RequestID=", ngx.ctx.reqid, " ",  "SAVE RESULT:" .. cjson.encode(commit_res) )
        if (commit_err == nil) then
            return true
        end
        ngx.log(ngx.WARN, "RequestID=", ngx.ctx.reqid, " ",  "Failed to write the key [", key, "] in Redis. Error:", commit_err)
        return false
    end
    ngx.log(ngx.WARN, "RequestID=", ngx.ctx.reqid, " ",  "Failed to save key:" .. tostring(key) .. " into cache: [", tostring(redis_host) .. ":" .. tostring(redis_port), "]. Error:", err)
    return false
end
...
function _M:evict(key)
    ngx.log(ngx.DEBUG, "RequestID=", ngx.ctx.reqid, " ",  "Delete key from Redis [", tostring(key), "]")
    if (key == nil or #key == 0) then
        ngx.log(ngx.WARN, "RequestID=", ngx.ctx.reqid, " ",  "Could not evict an empty key")
        return
    end
    local redis_rw = redis:new()
    local redis_host, redis_port = getRedisUpstream(REDIS_RW_UPSTREAM)
    ...
    if ok then
        ...
        ngx.log(ngx.WARN, "RequestID=", ngx.ctx.reqid, " ",  "Failed to delete key [", key, "] in Redis. Error:", commit_err)
        return false
    end
    ngx.log(ngx.WARN, "RequestID=", ngx.ctx.reqid, " ",  "Failed to delete key:" .. tostring(key) .. " from cache: [", tostring(redis_host) .. ":" .. tostring(redis_port), "]. Error:", err)
    return false
end
