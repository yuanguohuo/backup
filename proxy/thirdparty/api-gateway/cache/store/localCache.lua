...
function _M:getDictInstance()
    ...
    if dict == nil then
        ngx.log(ngx.WARN, "RequestID=", ngx.ctx.reqid, " ",  "dict `", tostring(self:getDict()), "` not defined. Please define it with 'lua_shared_dict ", tostring(self:getDict()), " 50m'")
        return nil
    end
    return dict
end
...
function _M:put(key, value)
    if (type(value) ~= "string") then
        ngx.log(ngx.WARN, "RequestID=", ngx.ctx.reqid, " ", ".Could not save key=", tostring(key), " into ", tostring(self:getName()), ". Invalid value of type=", type(value))
    end
    local d = self:getDictInstance()
    if (d ~= nil) then
        ...
        if (err) then
            ngx.log(ngx.WARN, "RequestID=", ngx.ctx.reqid, " ",  "Could not save key=", tostring(key), " into ", tostring(self:getName()), ".", err)
        end
        if (forcible) then
            ngx.log(ngx.INFO, "RequestID=", ngx.ctx.reqid, " ",  "shared dict=", tostring(self:getDict()) " has removed other items from memory when adding key:", tostring(key))
        end
