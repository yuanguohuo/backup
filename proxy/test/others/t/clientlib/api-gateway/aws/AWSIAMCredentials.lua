...
function AWSIAMCredentials:new(o)
    ...
    if (o ~= nil) then
        ...
        ngx.log(ngx.DEBUG, "RequestID=", ngx.ctx.reqid, " ",  "Initializing AWSIAMCredentials with object:", s)
    end
    return o
end
...
function AWSIAMCredentials:loadCredentialsFromSharedDict()
    ...
    if (iamCreds ~= nil) then
        ...
        ngx.log(ngx.DEBUG, "RequestID=", ngx.ctx.reqid, " ",  "Cache has been loaded from Shared Cache" )
    end
end
...
function AWSIAMCredentials:fetchIamUser()
    ngx.log(ngx.DEBUG, "RequestID=", ngx.ctx.reqid, " ",  "Fetching IAM User from:",
    ...
    if (code == ngx.HTTP_OK and body ~= nil) then
        cache.IamUser = body
        ngx.log(ngx.DEBUG, "RequestID=", ngx.ctx.reqid, " ",  "found user:" .. tostring(body))
        return cache.IamUser
    end
    ngx.log(ngx.WARN, "RequestID=", ngx.ctx.reqid, " ",  "Could not fetch iam user from:", self.security_credentials_host, ":", self.security_credentials_port, self.security_credentials_url)
    return nil
end
...
function AWSIAMCredentials:getIamUser()
    ...
    ngx.log(ngx.WARN, "RequestID=", ngx.ctx.reqid, " ",  "No iam_user provided. To improve performance please define one to avoid extra round trips to AWS")
...
function AWSIAMCredentials:fetchSecurityCredentialsFromAWS()
    ...
    ngx.log(ngx.DEBUG, "RequestID=", ngx.ctx.reqid, " ",  "AWS Response:" .. tostring(body))
    ...
    ngx.log(ngx.WARN, "RequestID=", ngx.ctx.reqid, " ",  "Could not read credentials from:", self.security_credentials_host, ":", self.security_credentials_port, iamURL)
    return false
end
...
function AWSIAMCredentials:getSecurityCredentials()
    ...
    if (cache.Token == nil or cache.SecretAccessKey == nil or cache.AccessKeyId == nil) then
        ngx.log(ngx.DEBUG, "RequestID=", ngx.ctx.reqid, " ",  "Obtaining a new token as the cache is empty.")
    ...
    if (now_in_secs >= expireAtTimestamp) then
        ngx.log(ngx.WARN, "RequestID=", ngx.ctx.reqid, " ",  "Current token expired " .. tostring(expireAtTimestamp - now_in_secs) .. " seconds ago. Obtaining a new token.")
        self:fetchSecurityCredentialsFromAWS()
    end
