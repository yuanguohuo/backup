...
function _M:getCredentialsFromSTS()
    ...
    if (code ~= ngx.HTTP_OK) then
        ngx.log(ngx.WARN, "RequestID=", ngx.ctx.reqid, " ",  "Could not get STS credentials. Response Code=", tostring(code), ", response body=", tostring(body))
        return nil
    end
...
function _M:getSecurityCredentials()
    ...
    if (stsCreds ~= nil) then
        ...
        if (now_in_secs >= expireAtTimestamp) then
            ngx.log(ngx.WARN, "RequestID=", ngx.ctx.reqid, " ",  "Current STS token expired " .. tostring(expireAtTimestamp - now_in_secs) .. " seconds ago. Obtaining a new token.")
        else
