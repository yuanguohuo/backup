...
local function loadrequire(module)
    ngx.log(ngx.DEBUG, "RequestID=", ngx.ctx.reqid, " ",  "Loading module [" .. tostring(module) .. "]")
    ...
    if not (res) then
        ngx.log(ngx.WARN, "RequestID=", ngx.ctx.reqid, " ",  "Could not load module [", module, "].")
        return nil
    end
...
function _M:new(o)
    ngx.log(ngx.DEBUG, "RequestID=", ngx.ctx.reqid, " ",  "AwsService() supercls=", tostring(o.___super))
    local o = o or {}
    setmetatable(o, self)
    self.__index = self
...
function _M:constructor(o)
    ngx.log(ngx.DEBUG, "RequestID=", ngx.ctx.reqid, " ",  "AwsService() constructor ")
    local s = tableToString(o)
    ngx.log(ngx.DEBUG, "RequestID=", ngx.ctx.reqid, " ",  "init object=" .. s)
...
function _M:getCredentialsProvider(init_object)
    ...
    if (init_object.aws_credentials == nil) then
        ngx.log(ngx.DEBUG, "RequestID=", ngx.ctx.reqid, " ",  "Missing 'aws_credentials' init option; will try to guess one.")
        ...
        if (key == "" or secret == "") then
            ngx.log(ngx.DEBUG, "RequestID=", ngx.ctx.reqid, " ",  "Using AWSIAMCredentials as 'aws_access_key' or 'aws_secret_key' were not provided.")
            init_object.aws_credentials = {
                provider = "api-gateway.aws.AWSIAMCredentials",
    ...
    ngx.log(ngx.DEBUG, "RequestID=", ngx.ctx.reqid, " ",  "Initializing '", tostring(init_object.aws_credentials.provider), "' credentials provider for aws service=", tostring(init_object.aws_service))
    local credentialsCls = loadrequire(init_object.aws_credentials.provider)
    if (credentialsCls == nil) then
        ngx.log(ngx.WARN, "RequestID=", ngx.ctx.reqid, " ",  "Invalid credentials provider:", tostring(init_object.aws_credentials.provider))
    end
    local credentialsProviderInstance = credentialsCls:new(init_object.aws_credentials)
    ngx.log(ngx.DEBUG, "RequestID=", ngx.ctx.reqid, " ",  "Initialized security provider '", tostring(init_object.aws_credentials.provider), "' with options:", tableToString(init_object.aws_credentials))
...
function _M:debug(...)
    if debug_mode then
        ngx.log(ngx.DEBUG, "RequestID=", ngx.ctx.reqid, " ",  "AwsService: ", ...)
    end
end
...
function _M:getCredentials()
    ...
    ngx.log(ngx.DEBUG, "RequestID=", ngx.ctx.reqid, " ",  tostring(self.aws_service) .. " uses credentials from:" .. tostring(self.aws_credentials_provider.provider) .. " ->" .. return_obj.aws_access_key, " >> ", return_obj.aws_secret_key, " >> ", return_obj.token)
    return return_obj
end

function _M:getAuthorizationHeader(http_method, path, uri_args, body, headers)
    ...
    ngx.log(ngx.DEBUG, "RequestID=", ngx.ctx.reqid, " ", "getauthheader :" .. http_method)
    local bodyhash ,authorization = awsAuth:getAuthorizationHeader(http_method,
        path, -- "/"
        uri_args, -- ngx.req.get_uri_args()
        body,
	headers)
    return bodyhash, authorization, awsAuth, credentials.token
end
...
function _M:getRequestArguments(actionName, parameters)
    ...
    if actionName then	
       urlencoded_args = "Action=" .. actionName
    end

    if parameters ~= nil then
        for key, value in pairs(parameters) do
            local proper_val = ngx.re.gsub(tostring(value), "&", "%26", "ijo")
	    if not urlencoded_args then
               urlencoded_args = key .. "=" .. (proper_val or "")
           else   
...
--
function _M:performAction(actionName, arguments, path, http_method, useSSL, timeout, contentType, extra_headers, request_body)
--    local host = self:getAWSHost()
-- todo using domain name 
    local host = "127.0.0.1"
    local request_method = http_method or "GET"

    ngx.log(ngx.DEBUG, "RequestID=", ngx.ctx.reqid, " ",  "perf args:" .. "request_method, " .. http_method ..", request_path,".. path .. ", uri_agrs," .. inspect(arguments) .. ",request_body ," .. request_body)
    ...
    local uri_args = arguments
    if actionName then
       uri_args.Action = actionName
    end
    local content_type = contentType or "application/x-amz-json-1.1"
    ngx.log(ngx.DEBUG, "RequestID=", ngx.ctx.reqid, " ",  "request_payload in perfaction funct before is:\n" .. request_body)
...
--    local tosign_payload = true
--    if request_headers["X-AMZ-Content-Sha256"] == "UNSIGNED-PAYLOAD"  then
--	    tosign_payload = false
--    end	    

    ngx.log(ngx.DEBUG, "RequestID=", ngx.ctx.reqid, " ",  "request_payload in perfaction funct is:\n" .. request_body)
    ngx.log(ngx.DEBUG, "RequestID=", ngx.ctx.reqid, " ",  "get auth heaer:" .. "request_method, " .. request_method ..", request_path,".. request_path .. ", uri_agrs," .. 
	    inspect(uri_args) .. ", request_body," .. request_body)
    local bodyhash, authorization, awsAuth, authToken = self:getAuthorizationHeader(request_method, request_path, uri_args, request_body, request_headers)

--    local t 
--    if actionName then
--	  t =  self.aws_service_name .. "." .. actionName
--    end
--    local request_headers = {
--        Authorization = authorization,
--        ["X-Amz-Date"] = awsAuth.aws_dateh,
...
--       request_headers["X-Amz-Date"] = awsAuth.aws_dateh
    ...
    if request_method == "GET" and query_string then
        request_path = request_path .. "?" .. query_string
    end

    if (self.aws_debug == true) then
        ngx.log(ngx.DEBUG, "RequestID=", ngx.ctx.reqid, " ",  "Calling AWS:", request_method, " ", scheme, "://", host, ":", port, request_path, ". Body=", request_body)
        local s = tableToString(request_headers)
        ngx.log(ngx.DEBUG, "RequestID=", ngx.ctx.reqid, " ",  "Calling AWS: Headers:", s)
    end
	...
	--keepalive = self.aws_conn_keepalive or 30000, -- 30s keepalive
        keepalive = self.aws_conn_keepalive or 30000000, -- 30s keepalive
        --poolsize = self.aws_conn_pool or 100 -- max number of connections allowed in the connection pool
        poolsize = self.aws_conn_pool or 500 -- max number of connections allowed in the connection pool
    }))

    if (self.aws_debug == true) then
        local s = tableToString(headers)
        ngx.log(ngx.DEBUG, "RequestID=", ngx.ctx.reqid, " ",  "AWS Response:", "code=", code, ", headers=", s, ", status=", status, ", body=", body)
    end
