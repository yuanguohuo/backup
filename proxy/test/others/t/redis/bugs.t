...
--- config
    ...
    location /foo {
        access_by_lua '
            ...
            -- ngx.log(ngx.ERR, "RequestID=", ngx.ctx.reqid, " ",  "hello");
            ...
            if not ok then
                ngx.log(ngx.ERR, "RequestID=", ngx.ctx.reqid, " ",  "failed to connect: ", err)
                return
            end

            res, err = red:set("dog", "an animal")
            if not res then
                ngx.log(ngx.ERR, "RequestID=", ngx.ctx.reqid, " ",  "failed to set dog: ", err)
                return
            end
            ...
            if err then
                ngx.log(ngx.ERR, "RequestID=", ngx.ctx.reqid, " ",  "failed to get dog: ", err)
                return
            end

            if not res then
                ngx.log(ngx.ERR, "RequestID=", ngx.ctx.reqid, " ",  "dog not found.")
                return
            end
            ...
            if not ok then
                ngx.log(ngx.ERR, "RequestID=", ngx.ctx.reqid, " ",  "failed to set keepalive: ", err)
                return
            end
